	...
	const std::string& fragmentPath)
{
	...
	std::shared_ptr<Renderer::ShaderProgram>& newShader = m_shaderPrograms
		.emplace(shaderName, std::make_shared<Renderer::ShaderProgram>(vertexString, fragmentString)).first->second;
																  ...
																  const std::string& texturePath)
{
	...
	std::shared_ptr<Renderer::Texture2D> newTexture = 
		m_textures2D.emplace(textureName, std::make_shared<Renderer::Texture2D>(widht,
																				height,
																				pixels,
																				channels,
																				GL_NEAREST,
...
{
	TexturesMap::const_iterator it = m_textures2D.find(textureName);
